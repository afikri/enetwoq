import React from 'react'
import MyScrollToTop from '../../components/ScrollToTop'

const Prince2AgileFoundation = () => {
  return (
    <div>

      <MyScrollToTop />

      <section id="partners" style={{
        backgroundImage: "linear-gradient(110deg, #171F32 47%, #171F3229 100%)",
        height: "60vh",
        fontFamily: "Poppins",

      }}>
        <div className="container" >
          <div className="col-lg-12" style={{
            margin: "auto",
            width: "100%",
            padding: "10px",
            color: "#FFF"
          }}>

            <h1 style={{ fontFamily: "Open Sans", }}><strong>PRINCE2 Agile® Foundation Certification Training Course</strong></h1><br />
            <div style={{ width: "800px" }}>
              <div className="col-sm">
                <div className="mt-5 d-flex align-items-center" style={{ justifyContent: "space-between" }}>
                  PRINCE2 Agile® Foundation
                  <div className="small-ratings" >
                    <i className="fa fa-star rating-color"></i>
                    <i className="fa fa-star rating-color"></i>
                    <i className="fa fa-star rating-color"></i>
                    <i className="fa fa-star rating-color"></i>
                    <i className="fa fa-star rating-color"></i>
                  </div>&nbsp;
                  <div style={{ paddingLeft: "155px", alignContent: "right" }}>
                    4148 Ratings&nbsp; |
                    17990 Learners
                  </div>
                </div>
              </div>
              <br />
              <div style={{ textAlign: "justify" }}>
                <p>
                  PRINCE2 Agile® is the world’s most complete project management solution, combining the flexibility and responsiveness of agile with the clearly defined framework of PRINCE2®.
                  Equip yourself with a highly-demanded certification in Project Management.

                </p>
              </div>
              <div style={{ paddingTop: "25px" }}>
                <p>
                  PRINCE2 Agile® is a registered trade mark of AXELOS Limited, used under permission of AXELOS Limited, all rights reserved. <br />
                  The Swirl logo™ is a trade mark of AXELOS Limited, used under permission of AXELOS Limited. All rights reserved.
                </p>
              </div>
            </div>
          </div>
        </div>
      </section>

      <section id="certifications">
        <div className="container">
          <div className="section-header">
            <h4 style={{
              fontFamily: "Montserrat",
              fontSize: "24px",
              color: "#54595F",
              textTransform: "uppercase",
              fontWeight: "700",
              position: "relative",
              paddingBottom: "20px"
            }}>Courses and Overview</h4>
            <div className="col-md-12 content" >
              <div className="col-sm" style={{ paddingBottom: "10px" }}>
                <div style={{ textAlign: "justify", display: 'grid', gridTemplateColumns: "100%" }}>
                  <p>
                    The PRINCE2 Agile® Foundation Certification is suitable for individuals who want to demonstrate they have sufficient
                    understanding of the PRINCE2 methodology and can work effectively as part of a team or with a project management team within a PRINCE2 supporting environment.
                  </p>
                </div>
              </div>
            </div>
          </div>
          <hr />
        </div>
      </section >

      <section id="certifications">
        <div className="container">
          <div className="section-header">
            <h2 style={{
              fontFamily: "Montserrat",
              fontSize: "24px",
              color: "#54595F",
              // textTransform: "uppercase",
              fontWeight: "700",
              position: "relative",
              paddingBottom: "20px"
            }}>
              PRINCE2 Agile® Foundation Certification
            </h2>
            <div className="col-lg-12 content" >
              <p>
                The PRINCE2 Agile Foundation certificate is designed to help professionals deliver agile projects by tailoring PRINCE2 management controls with a broad toolset of agile delivery techniques and frameworks.
              </p>

              <div className="col-sm" style={{ paddingBottom: "10px" }}>
                <div>
                  <p>The certification will help you to:</p>
                  <ul style={{ listStyle: "none" }}>
                    <li>
                      <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                      Understand PRINCE2 governance requirements clearly and comprehensively
                    </li>
                    <li>
                      <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                      Gain a broad range of knowledge of agile concepts and techniques including: Scrum, Kanban, Lean Start-up and Cynefin
                    </li>
                    <li>
                      <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                      Explore the interface between PRINCE2 and agile ways of working.
                    </li>
                  </ul>
                  <p>
                    This PRINCE2 Agile Foundation certification course provides a overview of Agile concepts, frameworks, and techniques. It also showcases PRINCE2 methods and processes. You’ll learn to establish a tailored governance framework within the Agile environment and focus on its management and delivery.
                  </p>

                </div>
                <div className="col-sm" style={{ paddingBottom: "10px" }}>
                  <div>
                    <h4 style={{ fontFamily: 'Poppins', fontSize: '18px' }}><b>Prerequisite:</b></h4>
                    <h4 style={{ fontFamily: 'Poppins', fontSize: '16px' }}>
                      No pre-requisites as this is entry level
                    </h4>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: "100%" }}>
                    <h4 style={{ fontFamily: 'Poppins', fontSize: '18px' }}><b>Examination Format</b></h4>
                    <div>
                      <ul style={{ listStyle: "none" }}>
                        <li>
                          <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          Multiple choice questions
                        </li>
                        <li>
                          <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          75 questions per paper
                        </li>
                        <li>
                          <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          Five questions to be trial and not counted in scores
                        </li>
                        <li>
                          <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          35 marks required (out of 70 available) to pass: 50%
                        </li>
                        <li>
                          <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          One hour's (60 minutes) duration
                        </li>
                        <li>
                          <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          Closed book
                        </li>
                      </ul>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <hr />
        </div>
      </section >



      <section id="certifications">
        <div className="container">
          <div className="section-header">
            <h2 style={{
              fontFamily: "Montserrat",
              fontSize: "24px",
              color: "#54595F",
              fontWeight: "700",
              position: "relative",
              paddingBottom: "20px"
            }}>PRINCE2® Foundation Training</h2>
            <div className="col-lg-12 content" style={{ textAlign: "justify" }}>
              <p>
                PRINCE2 training courses are run by accredited training organizations, in a variety of formats. eNetwoQ is part of ATO in Indonesia. The objective of this course is to provide the theoretical knowledge of how to plan, manage and deliver projects from start to finish. The PRINCE2 Foundation qualification is intended for project managers and aspiring project managers. The course covers widely used project management method that navigates you through all the essentials for running a successful project.
              </p>

              <div>
                <h4 style={{ fontFamily: 'Poppins', fontSize: '18px' }}><b>Training Syllabus</b></h4>
                <div style={{ display: 'grid', gridTemplateColumns: "50% 50%" }}>
                  <div>
                    <ul style={{ listStyle: "none" }}>
                      <li><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                        Introduction
                      </li>
                      <li><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                        PRINCE2 and Agile Concepts
                      </li>

                      <li><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                        Blending PRINCE2 and Agile
                      </li>
                      <li><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                        PRINCE2 Agile Focus Areas
                      </li>
                      <li><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                        What to Fix and What to Flex?
                      </li>
                      <li><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                        PRINCE2 Agile Guidance, Tailoring, and, Techniques
                      </li>
                    </ul>
                  </div>
                  <div>
                    <ul style={{ listStyle: "none" }}>
                      <li>
                        <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                        PRINCE2 Agile Foundation Tips and Tricks

                      </li>
                      <li>
                        <i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                        PRINCE2 Agile Foundation Simulation Exam
                      </li>
                    </ul>
                  </div>
                </div>
                <div>
                  <h4 style={{ fontFamily: 'Poppins', fontSize: '18px' }}><b>Facilitator :</b></h4>
                  <p>
                    PRINCE2 Agile Foundation Approved Trainer and Project management practitioner with over 12 years of experience in industry and holding valid PRINCE2 Agile certification</p>
                </div>
              </div>
            </div>
          </div>
          <hr />
        </div>
      </section >


      <section id="certifications">
        <div className="container">
          <div className="section-header">
            <div className="row gy-4">

              <h4 style={{ fontFamily: "Poppins", fontWeight: "900", color: "#54595F" }}>
                Our PRINCE2 Agile® Foundation Products
              </h4>
              <div className="row">
                <div className="col-md-6">
                  <div className="card mb-3 h-100">
                    <div className="card-header text-white" style={{ background: "#54595F", height: "110px", textAlign: 'center' }}>
                      <h4>PRINCE2 Agile® Foundation Full Package</h4>
                      <div>Training + Exam</div>
                    </div>
                    <div className="card-body" style={{ textAlign: "justify", background: "#F7F7F7" }}>
                      <ul style={{ listStyle: "none" }}>
                        <li style={{ borderBottom: "2px dotted #54595F" }}><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          <span className="list-name">Include Exam</span>
                        </li>
                        <li style={{ borderBottom: "2px dotted #54595F" }}><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          <span className="list-name">Online by Zoom</span>
                        </li>
                        <li style={{ borderBottom: "2px dotted #54595F" }}><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          <span className="list-name">Module Training in Softcopy </span>
                        </li>
                        <li style={{ borderBottom: "2px dotted #54595F" }}><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          <span className="list-name">Supporting Document in Softcopy </span>
                        </li>
                        <li style={{ borderBottom: "2px dotted #54595F" }}><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          <span className="list-name">Life time consultancy after training (WAG) </span>
                        </li>

                        <li style={{ borderBottom: "2px dotted #54595F" }}><i style={{ fontSize: "20px", paddingRight: "15px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          <span className="list-name">PRINCE2® Foundation Simulation </span>
                        </li>
                      </ul>
                    </div>
                  </div>
                </div>

                <div className="col-md-6">
                  <div className="card mb-3 h-100">
                    <div className="card-header text-white" style={{ background: "#54595F", height: "110px", textAlign: 'center' }}>
                      <h4>PRINCE2 Agile® Foundation Corporate Training</h4>
                      <div>Customized to your team’s need</div>
                    </div>
                    <div className="card-body" style={{ textAlign: "justify", background: "#F7F7F7" }}>
                      <ul style={{ listStyle: "none" }}>
                        <li style={{ borderBottom: "2px dotted #54595F" }}><i style={{ fontSize: "20px", paddingRight: "4px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          <span className="list-name">Customized Online/Offline</span>
                        </li>
                        <li style={{ borderBottom: "2px dotted #54595F" }}><i style={{ fontSize: "20px", paddingRight: "4px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          <span className="list-name">Module Training in Softcopy</span>
                        </li>
                        <li style={{ borderBottom: "2px dotted #54595F" }}><i style={{ fontSize: "20px", paddingRight: "4px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          <span className="list-name">Supporting Document in Softcopy</span>
                        </li>
                        <li style={{ borderBottom: "2px dotted #54595F" }}><i style={{ fontSize: "20px", paddingRight: "4px", color: "#50d8af", fontWeight: "400", }} className="bi bi-check-circle"></i>
                          <span className="list-name">Life time consultacy after training (WAG)</span>
                        </li>
                      </ul>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section >

    </div >
  )
}

export default Prince2AgileFoundation